# =========================================================================
# ✅ HC vs PD (IOWA) — DenseNet-121 (Pretrained, Last Layer Only)  [v1]
...
#  - Only the final classification head (model.classifier) is trainable.
#  - DenseNet-121 head: single Linear(1024 → 1) replacing the default
#    Linear(1024 → 1000).
#
...
#    No Dropout2d (backbone frozen; conv layers not updated)
# =========================================================================
...
BASE_RESULT_DIR = "/scratch/user/u.aa346327/results/densenet121"
...
# ====================== MODEL ======================
# DenseNet-121: freeze entire backbone, replace classifier with Linear(1024→1).
# model.classifier is a single nn.Linear — direct replacement.
def create_model():
    model = densenet121(weights=DenseNet121_Weights.IMAGENET1K_V1)
    for param in model.parameters():
        param.requires_grad = False
    in_features     = model.classifier.in_features   # 1024
    model.classifier = nn.Linear(in_features, 1)     # newly created → requires_grad=True
...
def write_summary_excel(gkf_fold_df, std_fold_df,
    ...
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        ...
        n_rows = len(df_gkf) + 1
        style_worksheet(writer.sheets["GroupKFold"], "1F3864",
        ...
        n_rows = len(df_std) + 1
        style_worksheet(writer.sheets["Standard5Fold"], "2F5496",
                        mean_row_idx=n_rows - 1, total_row_idx=n_rows)
...
def run_groupkfold(paths, labels, groups, tag, result_dir, model_dir, plot_dir):
    ...
    save_history_xlsx(histories,
                      os.path.join(result_dir, f"{tag}_groupkfold_history.xlsx"), "1F3864")
...
def run_standard_kfold(paths, labels, groups, tag, result_dir, model_dir, plot_dir):
    ...
    save_history_xlsx(histories,
                      os.path.join(result_dir, f"{tag}_standard_history.xlsx"), "2F5496")
...
for tag, hc_dir, pd_dir in DATASETS:
    print("\n" + "#"*70)
    print(f"#  DATASET: {tag}  (IOWA)  [DenseNet-121]")
    print("#"*70)
...
print("\n" + "#"*70)
print("#  ALL DATASETS COMPLETE  [DenseNet-121]  (EWT · CWT · STFT · DWT)")
print("#"*70)
