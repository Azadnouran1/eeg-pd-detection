# =========================================================================
# ✅ HC vs PD (IOWA) — ResNet-50 (Pretrained, Last Layer Only)  [v1]
...
#  - Only the final fully-connected head (model.fc) is trainable.
#  - ResNet-50 head: single Linear(2048 → 1) replacing the default
#    Linear(2048 → 1000).
#
...
BASE_RESULT_DIR = "/scratch/user/u.aa346327/results/resnet50"
...
# ====================== MODEL ======================
# ResNet-50: freeze entire backbone, replace model.fc with Linear(2048→1).
# model.fc is a single nn.Linear — direct replacement.
def create_model():
    model = resnet50(weights=ResNet50_Weights.IMAGENET1K_V1)
    for param in model.parameters():
        param.requires_grad = False
    in_features = model.fc.in_features   # 2048
    model.fc    = nn.Linear(in_features, 1)  # newly created → requires_grad=True
...
def write_summary_excel(gkf_fold_df, std_fold_df,
    ...
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        ...
        n_rows = len(df_gkf) + 1
        style_worksheet(writer.sheets["GroupKFold"], "833C00",
        ...
        n_rows = len(df_std) + 1
        style_worksheet(writer.sheets["Standard5Fold"], "C55A11",
                        mean_row_idx=n_rows - 1, total_row_idx=n_rows)
...
def run_groupkfold(paths, labels, groups, tag, result_dir, model_dir, plot_dir):
    ...
    save_history_xlsx(histories,
                      os.path.join(result_dir, f"{tag}_groupkfold_history.xlsx"), "833C00")
...
def run_standard_kfold(paths, labels, groups, tag, result_dir, model_dir, plot_dir):
    ...
    save_history_xlsx(histories,
                      os.path.join(result_dir, f"{tag}_standard_history.xlsx"), "C55A11")
...
for tag, hc_dir, pd_dir in DATASETS:
    print("\n" + "#"*70)
    print(f"#  DATASET: {tag}  (IOWA)  [ResNet-50]")
    print("#"*70)
...
print("\n" + "#"*70)
print("#  ALL DATASETS COMPLETE  [ResNet-50]  (EWT · CWT · STFT · DWT)")
print("#"*70)
